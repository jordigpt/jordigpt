...
// Tipado básico alineado con la DB
interface Product {
  id: string;
  title: string;
...
const Admin = () => {
  ...
  const [formData, setFormData] = useState<Partial<Product>>({
    ...
    is_featured: false,
    image_type: "chart-line-up" // default icon
  });
  ...
  if (loading && !products.length) return <div className="flex justify-center items-center h-screen">Loading...</div>;
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea id="short_description" name="short_description" value={formData.short_description} onChange={handleInputChange} />
                  </div>

                  <div>
                    <Label htmlFor="full_description">Full Description</Label>
                    <Textarea id="full_description" name="full_description" value={formData.full_description} onChange={handleInputChange} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input id="price_display" name="price_display" value={formData.price_display} onChange={handleInputChange} placeholder="US$ 29.99" />
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="original_price_label">Orig. Price Label</Label>
                      <Input id="original_price_label" name="original_price_label" value={formData.original_price_label} onChange={handleInputChange} />
                    </div>
                    <div>
                      <Label htmlFor="original_price_display">Orig. Price Display</Label>
                      <Input id="original_price_display" name="original_price_display" value={formData.original_price_display} onChange={handleInputChange} />
                    </div>
                  </div>

                   <div>
                    <Label htmlFor="price_microcopy">Price Microcopy</Label>
                    <Input id="price_microcopy" name="price_microcopy" value={formData.price_microcopy} onChange={handleInputChange} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                       ...
                       <Input id="cta_text" name="cta_text" value={formData.cta_text} onChange={handleInputChange} />
                    </div>
                    <div>
                       <Label htmlFor="badge">Badge</Label>
                       <Input id="badge" name="badge" value={formData.badge} onChange={handleInputChange} />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {formData.image_url && (
                        <div className="mt-2">
                            ...
                            <img src={formData.image_url} alt="Preview" className="h-20 w-auto rounded border border-border" />
                        </div>
                    )}
                  </div>
                  
                  <div>
                    <Label htmlFor="image_type">Icon Fallback (if no image)</Label>
                    <select 
                        id="image_type" 
                        name="image_type" 
                    ...
                    >
                        <option value="chart-line-up">Zap Icon (Chart)</option>
                        <option value="infinity">Infinity Icon</option>
                        <option value="unlock">Lock/Unlock Icon</option>
                    </select>
                  </div>
                  ...
                  <div className="flex gap-4 pt-2">
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor="is_free" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                        Is Free?
                      </label>
                    </div>
                    <div className="flex items-center space-x-2">
                ...
                <Card key={product.id} className="overflow-hidden group hover:border-neon/50 transition-colors">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            ...
                            {product.is_featured && <span className="text-[10px] bg-neon text-black px-1.5 py-0.5 rounded font-bold uppercase">Featured</span>}
                            {product.is_free && <span className="text-[10px] bg-muted-foreground text-white px-1.5 py-0.5 rounded font-bold uppercase">Free</span>}
                        </div>
                    ...
                    <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                        ...
                        <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive/90 hover:bg-destructive/10" onClick={() => handleDelete(product.id)}>
