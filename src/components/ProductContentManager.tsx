...
export const ProductContentManager = ({ product, open, onOpenChange }: ProductContentManagerProps) => {
  ...
  const handleAddContent = async () => {
    ...
    if (error) {
      ...
      setNewContent({ content_type: 'file', title: '', content_url: '', content_text: '' });
      fetchContent();
    }
    setLoading(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>Add, remove, and reorder the deliverables for this product.</DialogDescription>
        </DialogHeader>
        
        <div className="grid md:grid-cols-2 gap-8 py-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="space-y-4 p-4 border rounded-lg sticky top-0 bg-background">
            ...
            <div>
              ...
              <Select value={newContent.content_type} onValueChange={(v) => setNewContent(p => ({...p, content_type: v as any}))}>
            ...
            {newContent.content_type === 'file' && (
              <div>
                ...
                {newContent.content_url && <p className="text-xs mt-2 text-muted-foreground truncate">URL: {newContent.content_url}</p>}
              </div>
            )}
            ...
            {newContent.content_type === 'video' && (
              <div>
                ...
                <Input placeholder="e.g., https://www.youtube.com/embed/..." value={newContent.content_url} onChange={e => setNewContent(p => ({...p, content_url: e.target.value}))} />
              </div>
            )}
            ...
            {newContent.content_type === 'text' && (
              <div>
                ...
                <Textarea className="h-32" value={newContent.content_text} onChange={e => setNewContent(p => ({...p, content_text: e.target.value}))} />
              </div>
            )}
            
            <Button onClick={handleAddContent} disabled={loading || isUploading} className="w-full">
              {loading ? <Loader2 className="animate-spin" /> : <><Plus className="mr-2 h-4 w-4" /> Add Content</>}
            </Button>
          </div>
          ...
          <div className="space-y-4">
            ...
            {content.length === 0 ? (
              <p className="text-muted-foreground text-sm text-center py-8">No content yet.</p>
            ) : (
              content.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-3 border rounded-lg bg-muted/30">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground truncate max-w-xs">{item.content_url || "Text Content"}</p>
                  </div>
